...
class NamesManagerTest < ActiveSupport::TestCase
  ...
  test "special case handling" do
    assert_nil NamesManager.handle_false_positives('1234')
    assert_nil NamesManager.handle_false_positives('#2393 state:resolved')
    assert_nil NamesManager.handle_false_positives('Fixes GH-15891')
    assert_nil NamesManager.handle_false_positives('related #13166')
    assert_nil NamesManager.handle_false_positives(" \t \n \f")
    assert_nil NamesManager.handle_false_positives('See rails ML, subject "Text::Format Licence Exception" on Oct 15, 2005')
    assert_nil NamesManager.handle_false_positives("RubyConf '05")
    assert_nil NamesManager.handle_false_positives('Includes duplicates of changes from 1.1.4 - 1.2.3')
    assert_nil NamesManager.handle_false_positives('update from Trac')
    assert_nil NamesManager.handle_false_positives('"rails.raise_exceptions"')
    assert_nil NamesManager.handle_false_positives("'rails.raise_exceptions'")
    assert_nil NamesManager.handle_false_positives(':format')
    assert_nil NamesManager.handle_false_positives('RC1')
    assert_nil NamesManager.handle_false_positives('state:committed #969')
    assert_nil NamesManager.handle_false_positives('status:committed #1008')
    assert_nil NamesManager.handle_false_positives('#https://rails.lighthouseapp.com/attachments/106066/0001-Ensure-SqlBypass-use-ActiveRecord-Base-connection.patch state:committed')
    assert_nil NamesManager.handle_false_positives('type="month"')
    assert_nil NamesManager.handle_false_positives('CVE-2013-2212')
    assert_nil NamesManager.handle_false_positives('association=(associate)')
    assert_nil NamesManager.handle_false_positives('master')
    assert_nil NamesManager.handle_false_positives('extras no-cache max-age public must-revalidate')
    assert_nil NamesManager.handle_false_positives('ci kip')
    assert_nil NamesManager.handle_false_positives('ci skp')
    assert_nil NamesManager.handle_false_positives('#{arel.where_sql}')
    assert_nil NamesManager.handle_false_positives('https://github.com/rails/rails/issues/11834')
    assert_nil NamesManager.handle_false_positives('WHERE ()')
